...
Instance::Instance(NativeWindow& Window)
 : m_Window(Window)
{}
...
static void KeyCallback(GLFWwindow *Window, int key, int scancode, int action, int mods)
{
    ...
    Handle->Graphs[Handle->CurrentRenderGraph].PushCamera.ViewProj = Handle->Graphs[Handle->CurrentRenderGraph].Cam.Data.ViewProjectionMatrix;
}
...
static void MouseScroolCallback(GLFWwindow *Window, double xOffset, double yOffset)
{
    ...
    Handle->Graphs[Handle->CurrentRenderGraph].Cam.ZoomLevel = std::max(Handle->Graphs[Handle->CurrentRenderGraph].Cam.ZoomLevel, 0.25f);
    Handle->Graphs[Handle->CurrentRenderGraph].Cam.ZoomLevel = std::min(Handle->Graphs[Handle->CurrentRenderGraph].Cam.ZoomLevel, 5.f);
    ...
    SetProjection(Handle->Graphs[Handle->CurrentRenderGraph].Cam, -AspectRatio * ZoomLevel, AspectRatio * ZoomLevel, -ZoomLevel, ZoomLevel);
    Handle->Graphs[Handle->CurrentRenderGraph].PushCamera.ViewProj = Handle->Graphs[Handle->CurrentRenderGraph].Cam.Data.ViewProjectionMatrix;
}
...
void Instance::load(const std::string& AppName, unsigned int width, unsigned int height)
{
   ...
   VkDebugUtilsMessengerCreateInfoEXT MessengerCreateInfo = {};
    ...
    if (vkCreateInstance(&createInfos, 0, &m_Handle) != VK_SUCCESS)
        return;
#ifdef _DEBUG
    PFN_vkCreateDebugUtilsMessengerEXT DebugUtilMessenger = reinterpret_cast<PFN_vkCreateDebugUtilsMessengerEXT>(vkGetInstanceProcAddr(m_Handle, "vkCreateDebugUtilsMessengerEXT"));
    if (DebugUtilMessenger)
        DebugUtilMessenger(m_Handle, &MessengerCreateInfo, 0, &m_DebugMessenger);
#endif
...
void Instance::destroy()
{
    ...
    for (auto& RenderG : Graphs)
        DestroyRenderGraph(vkContext.vkDevice.Handle, Allocator, RenderG);
...
#ifdef _DEBUG
    auto func = (PFN_vkDestroyDebugUtilsMessengerEXT)vkGetInstanceProcAddr(m_Handle,
...
void Instance::run(std::shared_ptr<std::deque<std::string>> SharedQueue)
{
    ...
    while (!ffWindowShouldClose(m_Window)) {
        ...
        if (!Graphs.empty())
            Render(vkContext, GraphConstruct.RenderPass, GraphConstruct.Framebuffers, vkRenderer, Graphs[CurrentRenderGraph], m_Window.WindowSize);
    }
}
