...
Batch newBatch(std::vector<Array> Data, VkPrimitiveTopology Topology)
{
    ...
    for (size_t i = 0; i < Data.size(); ++i) {
        ...
        n.Layouts.push_back(Layout);
        memcpy(n.BatchedMeshes.Data + Layout.offset, Data[i].Data, Layout.size);
    }
...
Vertex *FillCoordinates3DSpace(Vertex *v, glm::vec3 A, glm::vec3 B)
{
    ...
    v[10].x = B.x; v[11].x = B.x;
    v[10].y = A.y; v[11].y = B.y;
    v[10].z = B.z; v[11].z = B.z;

    v[12].x = B.x; v[13].x = A.x;
    v[12].y = B.y; v[13].y = B.y;
    v[12].z = B.z; v[13].z = B.z;

    v[14].x = A.x; v[15].x = A.x;
    v[14].y = B.y; v[15].y = A.y;
    v[14].z = B.z; v[15].z = B.z;

    // Link faces
    v[16].x = A.x; v[17].x = A.x;
    v[16].z = A.x; v[17].z = A.z;
    v[16].y = A.x; v[17].y = B.x;

    v[18].x = B.x; v[19].x = B.x;
    v[18].z = A.x; v[19].z = A.z;
    v[18].y = A.x; v[19].y = B.x;

    v[20].x = B.x; v[21].x = B.x;
    v[20].z = B.x; v[21].z = B.z;
    v[20].y = A.x; v[21].y = B.x;

    v[22].x = A.x; v[24].x = A.x;
    v[22].z = B.x; v[24].z = B.z;
    v[22].y = A.x; v[24].y = B.x;
...
BoundingBox ComputeBatchBoundingBox(Batch b, bool asRenderableArray)
{
    ...
    if (asRenderableArray) {
        if (bbox.B.z == bbox.A.z) {
            ...
            v = FillColorSpace(v, bbox.Vertices.ElementCount, 0.1, 0.5, 0.5, 0.4); // BoundingBox will be rendered in a blue color.
...
Batch addDataToBatch(Batch& b, JSON::SceneObject& Obj)
{
    ...
    for (size_t i = 0; i < Obj.Data.size(); ++i) {
        ...
        memcpy(nArray.Data + Layout.offset, Obj.Data[i].Data, Layout.size);
    }
