...
namespace gr
{
    ...
    Error Pipeline::initShaders(const VkDevice& device, const char *vertexShaderFilename, const char *fragmentShaderFilename)
    {
        if (m_shaders.init(device, vertexShaderFilename, fragmentShaderFilename))
            return Error::FUNCTION_FAILED;
        return Error::NONE;
    }
