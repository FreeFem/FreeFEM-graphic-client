...
int FindRightSceneObject(std::vector<SceneObject> Objects, json JSONObj)
{
    ...
    for (int ite = 0; ite < (int)Objects.size(); ++ite) {
        if (Objects[ite].DataType == DataType && Objects[ite].RenderPrimitive == RenderPrimitive)
            return ite;
    }
    return -1;
}
...
{
    Layout.MeshArrays.push_back(Obj);
}
...
void LogSceneLayout(const SceneLayout& Layout)
{
    ...
    for (auto& obj : Layout.MeshArrays) {
        message.append(std::string("\tGroup ") + std::to_string(i) + std::string(" with ") + std::to_string(obj.Data.size()) + std::string(" geometry "));
        message.append(std::string("[") + GetRenderTypeName(obj.RenderPrimitive) + std::string("] ") + std::string(":\n"));
        int j = 0;
        for (auto& Geo : obj.Data) {
            message.append(std::string("\t\tGeometry " + std::to_string(j) + std::string(" has ") + std::to_string(Geo.ElementCount) + std::string(" vertices.\n")));
            ++j;
        }
