...
/**
 * @brief Graphic pipeline, rendering a batch of buffers of the same time using
 * the same shaders. Hard coded Shaders loading for test purpose. (Will be
 * changed later)
 */
class Pipeline {
   public:
     ...
     *
     * @return FORCE_USE_RESULT Error - Returns Error::NONE if initilization is
     * successful. Will throw a warning at compilation if result isn't checked.
     */
     ...
     * @param const char *fragmentShaderFilename[in] - Fragment shaders file
     * name.
     *
     * @return FORCE_USE_RESULT Error - Returns Error::NONE if initilization is
     * successful. Will throw a warning at compilation if result isn't checked.
     */
    FORCE_USE_RESULT Error initShaders(const VkDevice &device, const char *vertexShaderFilename,
