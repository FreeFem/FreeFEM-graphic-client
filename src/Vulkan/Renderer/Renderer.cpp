...
static std::vector<VkCommandBuffer> newCommandBuffers(const VkDevice Device, const VkCommandPool CommandPool, uint32_t Count, const VkCommandBufferLevel CmdBufferLevels) {
    VkResult res;
...
static VkFence newFence(const VkDevice& Device) {
    ...
    VkFence Fence;
    if (vkCreateFence(Device, &createInfo, 0, &Fence) != VK_SUCCESS)
...
static VkSemaphore newSemaphore(const VkDevice Device) {
    ...
    if (vkCreateSemaphore(Device, &createInfo, 0, &Semaphore))
        return VK_NULL_HANDLE;
    return Semaphore;
}
...
Renderer NewRenderer(const VkDevice& Device, VkQueue *Queue, uint32_t QueueIndex, const VkSurfaceKHR& Surface, VkExtent2D Extent)
{
    ...
    for (uint i = 0; i < 2; ++i) {
        ...
        if (n.Frames[i].PresentFence == VK_NULL_HANDLE || n.Frames[i].Render  == VK_NULL_HANDLE || n.Frames[i].Acquire == VK_NULL_HANDLE || n.Frames[i].CmdBuffer == VK_NULL_HANDLE)
...
void Render(const Context& vkContext, const VkRenderPass RenderPass, std::vector<VkFramebuffer> Framebuffers, Renderer& vkRenderer, const RenderGraph& Graph, const VkExtent2D Extent)
{
    ...
    res = vkAcquireNextImageKHR(vkContext.vkDevice.Handle, vkContext.vkSwapchain.Handle, UINT64_MAX, currentFrame.Acquire, VK_NULL_HANDLE, &imageIndex);
    ...
    for (auto& Node : Graph.Nodes) {
        ...
        vkCmdPushConstants(currentFrame.CmdBuffer, Node.Layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(CameraUniform), &Graph.PushCamera);
