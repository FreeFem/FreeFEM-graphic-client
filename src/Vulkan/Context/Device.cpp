    ...
    for (uint32_t i = 0; i < queueCount; i += 1) {
        ...
        if (QueueFamilyPropertiesArray[i].queueFlags & VK_QUEUE_GRAPHICS_BIT && QueueFamilyIndices[0] == UINT32_MAX) {
            ...
            ++count;
                   ...
                   QueueFamilyIndices[1] == UINT32_MAX) {
...
static VkSampleCountFlagBits getMaxUsableSampleCount(Device& Device) {
    VkSampleCountFlags count =
        std::min(Device.PhysicalHandleCapabilities.Properties.limits.framebufferColorSampleCounts,
...
static void NewPhysicalDevice(const VkInstance Instance, const VkSurfaceKHR Surface, Device& Device) {
    ...
    if (vkEnumeratePhysicalDevices(Instance, &PhysicalDeviceCount, 0) != VK_SUCCESS) return;
    if (PhysicalDeviceCount < 1) return;
    std::vector<VkPhysicalDevice> PhysicalDevices(PhysicalDeviceCount);
    if (vkEnumeratePhysicalDevices(Instance, &PhysicalDeviceCount, PhysicalDevices.data( )) != VK_SUCCESS) return;
...
Device NewDevice(const VkInstance Instance, const VkSurfaceKHR Surface, std::vector<std::string> Layers) {
    ...
    if (n.QueueIndex[DEVICE_GRAPH_QUEUE] != n.QueueIndex[DEVICE_TRANS_QUEUE] &&
        n.QueueIndex[DEVICE_PRESENT_QUEUE] != n.QueueIndex[DEVICE_TRANS_QUEUE]) {
        queueCreateInfo.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
    ...
    if (vkCreateDevice(n.PhysicalHandle, &deviceCreateInfo, 0, &n.Handle)) return n;
