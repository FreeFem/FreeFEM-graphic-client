 ...
 *
 * @param const VkPhysicalDeviceMemoryProperties memoryProperties[in] - Physical device memory properties.
 * @param const uint32_t memoryTypeBits[in] - Memory type.
 * @param const VkMemoryPropertyFlags requiredMemoryProperties[in] - Required memory properties.
 *
 * @return int - returns -1 if function fails else returns the value of the memory type.
 */
